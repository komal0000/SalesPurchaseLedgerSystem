...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="{{ route('employee-salaries.create') }}" class="inline-flex items-center justify-center rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-indigo-700">New Salary</a>
        </div>
        ...
        <form method="GET" action="{{ route('employee-salaries.index') }}" class="grid gap-4 rounded-xl border border-gray-200 bg-white p-4 shadow-sm md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_auto] md:items-end">
            <div>
                ...
                <input type="text" name="employee_name" value="{{ $filters['employee_name'] ?? '' }}" placeholder="Search employee name" class="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2">
            </div>
                    ...
                    <tbody>
                        @forelse ($salaries as $salary)
                            <tr class="border-t border-gray-100 hover:bg-gray-50/70">
                                <td class="px-4 py-3">
                                    <p class="font-medium text-gray-900">{{ $salary->employee_name }}</p>
                                    <p class="text-xs text-gray-500">{{ $salary->employee_code ?: '-' }}</p>
                                </td>
