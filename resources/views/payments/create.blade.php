        ...
        <form action="{{ route('payments.store') }}" method="POST" class="mt-6 space-y-4">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div>
                    ...
                    <select id="account_id" name="account_id" class="select2 mt-1 w-full rounded-lg border border-gray-300 px-3 py-2">
                        ...
                        @foreach ($accounts as $account)
                            <option value="{{ $account->id }}" @selected(old('account_id') === $account->id)>{{ $account->name }} ({{ ucfirst($account->type) }})</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <select id="type" name="type" class="select2 mt-1 w-full rounded-lg border border-gray-300 px-3 py-2">
            ...
            <div class="rounded-lg bg-amber-50 px-4 py-3 text-sm text-amber-700">Link either a sale or a purchase. Leave both empty for an advance payment.</div>
